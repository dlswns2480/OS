...
fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  np->next = ptable.dummy.next;
  np->prev = &ptable.dummy;
  ptable.dummy.next->prev = np;
  ptable.dummy.next = np;
  np->state = RUNNABLE;
...
exit(void)
{
  ...
  for(p = ptable.dummy.next; p != &ptable.dummy; p = p->next){
    if(p->parent == curproc){
      p->parent = initproc;
      if(p->state == ZOMBIE)
        wakeup1(initproc);
  ...
  for(;;){
    ...
    for(p = ptable.dummy.next; p != &ptable.dummy; p = p->next){
      ...
      if(p->state == ZOMBIE){
        ...
        acquire(&ptable.lock);
        struct proc *child_pcb = ptable.proc + (p - ptable.proc);
        *child_pcb = (struct proc){0};
